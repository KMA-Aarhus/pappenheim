...
if [ -z "$1" ]; then
    ...
    #clear
    cd ~/pappenheim && touch CoverMon.flag && conda activate pappenheim && snakemake --profile default --use-conda --conda-frontend mamba --config samplesheet="${1}" rundir="${2}" ${3} && echo && cowsay "pappenheim finished successfully, you may now close this window."
fi
}

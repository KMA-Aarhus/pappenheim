...
if [ -z "$1" ]; then
    ...
    #clear
    cd ~/pappenheim && source activate pappenheim && snakemake --profile default --config samplesheet="${1}" rundir="${2}" ${3} && echo && cowsay "pappenheim finished successfully, you may now close the window."
fi
}

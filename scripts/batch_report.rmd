...
```{r echo=F, message=F, warning=F}
metadata_init$monitorering <- NA

for (i in 1:nrow(metadata_init)) {
  if (grepl("BA.2", metadata_init$pa_lineage[i]) & grepl("H78Y", metadata_init$ne_aaSubstitutions[i])) {
    metadata_init$monitorering[i] <-  "BA.2 + H78Y"
  }
  
  else if (grepl("BA.2", metadata_init$pa_lineage[i]) & grepl("L452R", metadata_init$ne_aaSubstitutions[i])) {
    metadata_init$monitorering[i] <-  "BA.2 + L452R" }
  
  else if (metadata_init$ne_totalMissing_interpreted[i] > 5000) {
    metadata_init$pa_lineage[i] <- "Unassigned"
  }
...
metadata_init %>% 
    ...
    mutate(mads_type = case_when(type == "sample" ~ str_sub(ya_sample_id, 1, 1), # Insert the sample type letter into the table.
                                 TRUE ~ "")) %>%
    select(barcode, rack_id, position, sample_id, type, pa_lineage, `totalMissing` = ne_totalMissing_interpreted, `numberOfReads` = ba_reads, monitorering) %>%
    knitr::kable()
...
### PCR-produkt
```{r echo=F, message=F, warning=F, fig.height = 4, fig.width = 7}
